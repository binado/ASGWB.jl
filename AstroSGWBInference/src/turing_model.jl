...
Identical to [`astrosgwb_importance_turing_model`](@ref) except that one strictly
...
BNS adapter these come from `AstroSGWBImportanceModels.bns_amplitude_scalings`.
...
log density matches [`astrosgwb_importance_turing_model`](@ref) exactly rather than up to
a constant.

The three σ-space contractions go through [`AstroSGWB.inner_product`](@ref), which already
carries the `2 T Δf` prefactor that makes it consistent with
[`AstroSGWB.gaussian_bin_scale`](@ref); a PSD-space contraction would make ``\\rho^2`` too
small by `2T`.
"""
@model function astrosgwb_amplitude_marginalized_turing_model(
        merger_rate_and_log_weights_fn,
        polarization_power::AbstractMatrix{<:Real},
        samples::NamedTuple,
        prior_model,
