"""
    forward_model(weights_fn, fluxes, samples, Λ; average_mode=AnalyticInclination())
...
the raw `(nfreq, nsamples)` flux matrix into the strain spectral density `Sₕ`.
...
`fluxes` (see [`AstroSGWB.spectral_density`](@ref)). It must match the mode `observed` was
...
function forward_model(
    ...
    return (; rate, weights,
        spectral_density = AstroSGWB.spectral_density(
            fluxes, rate; weights, average_mode))
end
